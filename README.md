# MIT courses
- [Introduction to Flow Matching and Diffusion Models 2025](https://diffusion.csail.mit.edu/2025/index.html)
  - [lecture notes](https://diffusion.csail.mit.edu/docs/lecture-notes.pdf)
  - [YouTube recording](https://www.youtube.com/watch?v=GCoP2w-Cqtg)

# Paper
- [SCORE-BASED GENERATIVE MODELING THROUGH STOCHASTIC DIFFERENTIAL EQUATIONS](https://arxiv.org/pdf/2011.13456)
- [SVGD as a kernelized Wasserstein gradient flow of the chi-squared divergence](https://arxiv.org/pdf/2006.02509)
- [Flow Straight and Fast: Learning to Generate and Transfer Data with Rectified Flow](https://arxiv.org/pdf/2209.03003)
